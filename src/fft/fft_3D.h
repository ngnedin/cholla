...
class FFT_3D
{
  public:
    FFT_3D();
    ~FFT_3D();
    void Initialize(Real lx, Real ly, Real lz, Real xMin, Real yMin, Real zMin, int nx, int ny,
                    int nz, int nxReal, int nyReal, int nzReal, Real dx, Real dy, Real dz);
    void Reset();
    
    void Filter_inv_k2( double *input, double *output, bool in_device ) const; 

    void Filter_rescale_by_power_spectrum( double *input, double *output, bool in_device, int size, 
                                           double *dev_k, double *dev_pk, Real dx3 ) const;
